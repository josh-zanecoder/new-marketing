...
mode="${1:?mode required: web|worker|send-worker|brevo-webhook}"
...
case "$mode" in
  web)
    # Injected at deploy when send-worker is live: CAMPAIGN_SEND_WORKER_URL + EMAIL_WORKER_DISABLED.
  ...
  worker)
    ...
    ;;
  send-worker)
    # Keep MARKETING_PUBLIC_BASE_URL / NUXT_PUBLIC_MARKETING_BASE_URL / MARKETING_APP_URL:
    # campaign batches build List-Unsubscribe and footer URLs on this service.
    skip_keys="KAFKA_INBOUND_CONSUMER_DISABLED|KAFKA_CRM_CONSUMER_DISABLED|EMAIL_WORKER_DISABLED|SCHEDULE_RECONCILE_DISABLED|SENDING_RECONCILE_DISABLED|KAFKA_INBOUND_TOPIC_REFRESH_MS|KAFKA_INBOUND_TOPIC_SIGNAL_MS|KAFKA_CONSUMER_SESSION_TIMEOUT_MS|KAFKA_CONSUMER_HEARTBEAT_INTERVAL_MS|KAFKA_CONSUMER_REBALANCE_TIMEOUT_MS|MARKETING_SYNC_RECIPIENT_LIST_CONCURRENCY|CAMPAIGN_SEND_WORKER_URL|CAMPAIGN_TASK_HANDLER_URL"
    ;;
  brevo-webhook)
...
case "$mode" in
  web)
    # BullMQ EmailWorker stays on unless the deploy workflow injects EMAIL_WORKER_DISABLED.
    ;;
  worker)
