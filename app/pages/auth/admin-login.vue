...
if (host.isTenantHost) await navigateTo('/auth/tenant-login')

const { email, password, loading, errorMessage, handleLogin } = useMarketingLogin('admin')
</script>
      ...
      <aside class="relative hidden overflow-hidden border-r border-slate-200 bg-white lg:flex lg:flex-col">
        <div class="relative flex-1 px-8 py-8">
          ...
          <div class="relative z-10 flex h-full items-center justify-center">
            <p class="text-6xl font-semibold tracking-wide text-rose-300/80">Marketing Admin</p>
          </div>
        </div>
      </aside>
        ...
        <form class="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm" @submit.prevent="handleLogin">
          ...
          <div class="mt-8 space-y-5">
            <div class="space-y-2">
              ...
              <input id="email" v-model="email" type="email" autocomplete="email" required placeholder="admin@company.com" class="w-full rounded-md border border-slate-200 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-rose-300 focus:ring-2 focus:ring-rose-100">
            </div>
            <div class="space-y-2">
