<template>
  <Teleport to="body">
    <div
      v-if="props.open && props.clientKey"
    ...
    >
      <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-slate-200/60">
        ...
        <p class="mt-3 text-xs text-slate-500">
          Use in requests: <code class="rounded bg-slate-100 px-1">x-client-key: YOUR_KEY</code>
...
async function copyToClipboard() {
  try {
    await navigator.clipboard.writeText(props.clientKey)
    copied.value = true
    setTimeout(() => { copied.value = false }, 2000)
  } catch {
    const input = document.createElement('input')
    input.value = props.clientKey
