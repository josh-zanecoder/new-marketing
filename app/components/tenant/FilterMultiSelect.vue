...
export type FilterMultiSelectOption = {
  value: string
  label: string
  /** Optional tooltip override; defaults to event-type tooltip for `value`. */
  tooltip?: string
}
...
const props = withDefaults(
  defineProps<{
    ...
    disabled?: boolean
    /** Label when nothing is selected (all). */
    allLabel?: string
...
/** Empty array = all (no filter). Non-empty = selected values. */
...
const selectedLabel = computed(() => {
  if (!model.value.length) return props.allLabel
  if (model.value.length === 1) {
    const match = props.options.find((option) => option.value === model.value[0])
    return match?.label ?? model.value[0]
  }
...
function optionTooltip(option: FilterMultiSelectOption): string {
  return option.tooltip?.trim() || brevoEventTypeTooltip(option.value)
}
...
<template>
  <div ref="rootRef" :class="rootClass">
    ...
    <Teleport v-if="open" to="body">
      <div
        ref="panelRef"
        class="fixed z-[200] flex flex-col rounded-xl border border-slate-200/90 bg-white shadow-lg shadow-slate-900/10 ring-1 ring-slate-900/[0.04]"
        :style="panelStyle"
        role="listbox"
      ...
      >
        <div
          v-if="searchable"
          class="sticky top-0 z-10 shrink-0 rounded-t-xl border-b border-slate-100 bg-white p-2"
        >
        ...
        <div class="min-h-0 flex-1 overflow-y-auto overscroll-contain overflow-x-visible py-1">
          <UiHoverTip :text="brevoEventTypeTooltip('all')" placement="right">
            <button
              type="button"
              ...
              :class="
                model.length === 0
                  ? 'bg-primary-50/80 font-semibold text-primary-900'
                  : 'font-medium text-slate-800'
              "
              role="option"
              :aria-selected="model.length === 0"
              :disabled="disabled"
              @click="selectAll"
            >
          ...
          <UiHoverTip
            ...
            :text="optionTooltip(option)"
            placement="right"
          >
            <button
              type="button"
              class="flex w-full min-w-0 items-center gap-2.5 px-4 py-2.5 text-left text-sm transition-colors hover:bg-slate-50 disabled:opacity-50"
              :class="
