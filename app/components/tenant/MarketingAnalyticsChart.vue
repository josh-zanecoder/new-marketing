...
const props = defineProps<{
  ...
  loading?: boolean
  /** When set, chart shows only the related series. */
  eventType?: string
}>()

const chartOption = computed(() =>
  buildMarketingAnalyticsChartOption(props.points, props.dateRange, props.eventType)
)
...
<template>
  ...
  >
    <div
      v-if="loading"
      class="flex h-56 items-center justify-center sm:h-80"
    >
    ...
    <ClientOnly v-else>
      <div v-if="hasChartData" class="px-1 pb-3 pt-2 sm:px-4 sm:pb-5 sm:pt-4">
        <VChart
          class="h-56 w-full min-h-[14rem] sm:h-80 sm:min-h-[20rem]"
          :option="chartOption"
          autoresize
        />
      </div>
      ...
      <template #fallback>
        <div class="flex h-56 items-center justify-center text-sm text-zinc-500 sm:h-80">
          Loading chart…
        </div>
      </template>
    </ClientOnly>
