...
import {
  isAdminAuthContext,
  isTenantClientAuthContext
} from './roles'

/** Matches tenant DB names produced by `toClientDbName` and typical Mongo naming. */
...
/**
 * Mongoose connection for the authenticated client's tenant database only.
...
export async function getTenantConnectionFromEvent(
  ...
  if (isAdminAuthContext(auth)) {
    throw createError({
      statusCode: 403,
      message: 'Tenant-scoped routes require a client session or client API key'
    })
  }
  if (!isTenantClientAuthContext(auth)) {
    throw createError({
      statusCode: 403,
 ...
 * Pass only `dbName` values that originated from trusted server state (e.g. job payload from enqueue), never raw client input.
 */
