...
export function peekUnsubscribeDbFromToken(token: string): string {
  return peekUnsubscribePayloadUnsafe(token)?.db?.trim() ?? ''
}
  ...
  switch (reason) {
    ...
    case 'tenant_not_configured':
      return {
        ...
        message: 'Email preferences are not configured for this organization. Please contact support.'
      }
